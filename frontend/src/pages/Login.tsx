...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { token, username: returnedUsername, riskScore } = response.data;
      
      // Store token and an object wrapper for user
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify({ username: returnedUsername, roles: ['ROLE_USER'] })); // Hardcoded default role since API doesn't return it
      
      navigate('/dashboard');
    } catch (err: any) {
      setError(err.response?.data?.error || 'Invalid username or password. Account may be locked.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-10 p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-white tracking-tight">Security SOC Login</h2>
          <p className="text-slate-400 text-sm mt-1">EURDS / NEC AI Intelligence Platform</p>
        </div>
